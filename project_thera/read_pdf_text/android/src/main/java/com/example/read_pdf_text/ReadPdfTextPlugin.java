...
public class ReadPdfTextPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        } else if (call.method.equals("getPDFtextForPage")) {
            final String path = call.argument("path");
            final int page = call.argument("page");
            parsePDFPageText(path, page);
        } else if (call.method.equals("getPDFtextForRange")) {
            final String path = call.argument("path");
            final int startPage = call.argument("startPage");
            final int endPage = call.argument("endPage");
            parsePDFPageRange(path, startPage, endPage);
    ...
    // This has to be done or else the Main Thread is blocked and user would close
    // the app.
    ...
    private void parsePDFPageText(String path, int page) {
        PdfPageAsyncTask task = new PdfPageAsyncTask(page);
        task.execute(path);
    }

    private void parsePDFPageRange(String path, int startPage, int endPage) {
        PdfPageRangeAsyncTask task = new PdfPageRangeAsyncTask(startPage, endPage);
        task.execute(path);
    }
    ...
    // [doInBackGround] is used for the parsing and [onPostExecute] is used to
    // return the text back to Main Thread
    private class PdfAsyncTask extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... strings) {
            ...
            PDDocument document = null;
            try {
                // The strings contains all arguments passed to the [AsyncTask], we only pass
                // one argument so we take the first item
    ...
    // Async task for single page extraction
    private class PdfPageAsyncTask extends AsyncTask<String, String, String> {
        private int pageNumber;

        public PdfPageAsyncTask(int pageNumber) {
    ...
    // Async task for page range extraction
    private class PdfPageRangeAsyncTask extends AsyncTask<String, String, String> {
        private int startPage;
        private int endPage;

        public PdfPageRangeAsyncTask(int startPage, int endPage) {
