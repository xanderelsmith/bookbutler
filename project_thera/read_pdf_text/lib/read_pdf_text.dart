...
class ReadPdfText {
  ...
  /// Returns text for a specific page (1-based index)
  static Future<String> getPDFtextForPage(String path, int pageNumber) async {
    final String pdfText = await _channel.invokeMethod('getPDFtextForPage',
        <String, dynamic>{'path': path, 'page': pageNumber});
    return pdfText;
  }

  /// Returns text for a range of pages (1-based index, inclusive)
  static Future<String> getPDFtextForRange(
      String path, int startPage, int endPage) async {
    final String pdfText = await _channel.invokeMethod(
        'getPDFtextForRange', <String, dynamic>{
      'path': path,
      'startPage': startPage,
      'endPage': endPage
    });
